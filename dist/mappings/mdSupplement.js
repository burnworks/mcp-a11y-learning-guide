...
function parseMd(filePath) {
    ...
    for (const raw of lines) {
        ...
        if (!entry)
            continue;
        const item = { id, title: label, url };
        if (id.startsWith("F")) {
            if (!entry.failures.find((f) => f.id === id))
                entry.failures.push(item);
        }
        else {
            if (!entry.techniques.find((t) => t.id === id))
                entry.techniques.push(item);
        }
    }
    return index;
}
export function loadMdSupplementIndex() {
    if (cachedIndex)
        return cachedIndex;
