...
const normalize = (value) => lower(value.trim());
...
function scoreMapping(query, mapping) {
...
function findMatchingTopics(query, mappings) {
...
function filterCriteria(criteria, priority) {
...
function filterTechniques(techniques, priority) {
...
function filterFailures(failures, priority) {
...
function formatRelatedTopics(related) {
    if (!related || related.length === 0)
        return "（見つかりませんでした）";
    return related.map((topic) => `• ${topic}`).join(newline);
}
function collectSupplement(criterionIds, existingTechIds, existingFailIds) {
...
function formatSupplement(items) {
    if (!items.length)
        return "";
    return items.map((i) => `• ${i.id}: ${i.title}\n  ${i.url}`).join(newline);
}
export async function searchAccessibilityTopic(query, priorityFilter, mappings) {
    ...
    const supplement = collectSupplement(mapping.references.criteria.map((c) => c.id), new Set(mapping.references.techniques.map((t) => t.id)), new Set(mapping.references.failures.map((f) => f.id)));
    const titleSuffix = priorityFilter === "must-read"
        ? " - 必読ガイド"
