...
function locateCriterion(
  id: string,
  mappings: TopicMapping[]
): { mapping: TopicMapping; criterion: Criterion } | null {
  const target = id.trim();

  for (const mapping of mappings) {
    const match = mapping.references.criteria.find((c: Criterion) => c.id === target);
    if (match) {
...
function formatTechniques(techniques: Technique[]) {
  if (!techniques.length) return "（なし）";
  return techniques
    .map((t) => {
...
export function getWcagCriterion(criterionId: string, mappings: TopicMapping[]): string {
