...
import { searchAccessibilityTopic, PriorityFilter } from "./tools/searchTopic.js";
...
const TOOL_DEFINITIONS = [
  {
    ...
    description:
      "アクセシビリティのトピックを検索し、関連する公式ドキュメント（WCAG、Understanding、Techniques）を提示します。学習者を公式資料へ導くガイド役として機能します。",
    inputSchema: {
      type: "object",
      properties: {
        query: {
...
let cachedMappings: TopicMapping[] | null = null;

async function getMappings(): Promise<TopicMapping[]> {
...
function coercePriorityFilter(value: unknown): PriorityFilter {
  const allowed: PriorityFilter[] = ["all", "must-read", "high-priority"];
  if (typeof value === "string" && (allowed as string[]).includes(value)) {
    return value as PriorityFilter;
  }
  return "all";
}
